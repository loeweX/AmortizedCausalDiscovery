...
class ChargedParticlesSim(object):
    ...
    def sample_trajectory(
        self, T=10000, sample_freq=10, charge_prob=[1.0 / 2, 0, 1.0 / 2], edges=None
    ):
        n = self.n_balls
        assert T % sample_freq == 0
        T_save = int(T / sample_freq - 1)
...
class LorenzSim(object):
    def __init__(self, p, F=10.0, delta_t=0.1, sd=0.1, burn_in=1000, seed=0):
        params = locals()
        del params["self"]
    ...
    def _lorenz(self, x, t, F):
        """Partial derivatives for Lorenz-96 ODE."""
        p = len(x)
        dxdt = np.zeros(p)
        for i in range(p):
            dxdt[i] = (
                (x[(i + 1) % self.p] - x[(i - 2) % self.p]) * x[(i - 1) % self.p]
                - x[i]
                + F
            )
    ...
    def sample_trajectory(self, T=10000, sample_freq=10):
        ...
        x0 = np.random.normal(scale=0.01, size=self.p)
        t = np.linspace(0, (T + self.burn_in) * self.delta_t, T + self.burn_in)
        X = odeint(self._lorenz, x0, t, args=(self.F,))

        dXdt = np.zeros(np.shape(X))
        for i in range(np.shape(X)[0]):
            dXdt[i] = self._lorenz(X[i], None, self.F)

        X += np.random.normal(scale=self.sd, size=(T + self.burn_in, self.p))

        # Set up Granger causality ground truth.
        GC = np.zeros((self.p, self.p), dtype=int)
        ...
        X = np.expand_dims(X[self.burn_in :], 1)
        return X, dXdt, GC
